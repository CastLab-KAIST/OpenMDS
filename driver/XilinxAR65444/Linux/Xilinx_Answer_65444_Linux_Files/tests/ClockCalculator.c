...
int main(int argc, char **argv) {
  ...
  printf("argc = %d\n", argc);
  printf("Used API: %s\n", argv[0]);
  //strdup 함수: 동적할당하는 heap 영역에 새로 메모리를 잡음
  device = strdup(argv[1]);
  printf("device: %s\n", device);
  //unsigned long int strtoul(const char *restrict str, char **endptr, int base): 숫자 문자열을 unsigned long형 숫자로 변환
  //str: 정수로 변환할 문자열, endptr: 숫자로 변경하지 못하는 문자열의 시작 위치, bast: 문자열이 가지고 있는 숫자의 진수
  //반환: unsigned long int 10진 long형 정수 값
    ...
    if (argc >= 5)
    {
      ...
      switch (access_width)
      {
        case 'w':
          ...
          printf("DIVCLK_DIVIDE = %f\n", DIVCLK_DIVIDE);
          printf("CLKFBOUT_MULT = %f\n", CLKFBOUT_MULT);
          printf("CLKOUT0_DIVIDE = %f\n", CLKOUT0_DIVIDE);
          cal_clk = cal_clk / DIVCLK_DIVIDE * CLKFBOUT_MULT / CLKOUT0_DIVIDE;
          printf("Calculated Clock = %f \n", cal_clk);
          printf("Desired Clock %d\n", (unsigned int)writeval);

          // Rule DIVCLK_DIVIDE = 1~106
          // Rule CLKFBOUT_MULT = (2.000~128.000) Fractional should be 0.125 단위
          // Rule CLKOUT0_DIVIDE = (1.000~128.000) Fractional should be 0.125 단위
          for(i = 2 ; i < 107 ; i++){
            for(k = 16 ; k < 1025 ; k++){
              for(j = 16 ; j < 1025 ; j++){
                temp_clk = 100 / i;
