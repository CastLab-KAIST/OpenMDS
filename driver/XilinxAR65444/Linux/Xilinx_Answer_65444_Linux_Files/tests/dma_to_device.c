...
static int test_dma(char *devicename, uint64_t addr, uint32_t size, uint64_t offset, uint32_t count, char *filename);
...
static uint32_t getopt_integer32(char *optarg)
{
  int rc;
  uint32_t value;
...
static uint64_t getopt_integer(char *optarg)
{
  int rc;
  uint64_t value;
  rc = sscanf(optarg, "0x%016llx", &value);
  if (rc <= 0)
    rc = sscanf(optarg, "%llx", &value);
  printf("sscanf() = %d, value = 0x%016llx\n", rc, (unsigned long long)value);
  return value;
}
...
int main(int argc, char* argv[])
{
  ...
  printf("device = %s, address = 0x%016llx, size = 0x%08x, offset = 0x%016llx, count = %u\n", device, address, size, offset, count);
...
//devicename: -d, addr: -a, size: -s, offset: -o, count: -c, filename: -f
static int test_dma(char *devicename, uint64_t addr, uint32_t size, uint64_t offset, uint32_t count, char *filename)
{
  int rc;
  char *buffer = NULL;
